...
export function createMatrixRoomMessageHandler(params: MatrixMonitorHandlerParams) {
  ...
  return async (roomId: string, event: MatrixRawEvent) => {
    try {
      ...
      // === KRILL PROTOCOL INTERCEPTION ===
      // Check if this is a Krill protocol message and handle it
      const messageBody = (event.content as RoomMessageEventContent)?.body;
      if (typeof messageBody === "string" && messageBody.startsWith("{")) {
        try {
          const krillResult = await interceptKrillMessage(
            client,
            roomId,
            senderId,
            messageBody,
          ...
          if (krillResult.handled) {
            logVerboseMessage(`matrix: krill message handled, not passing to agent`);
            return; // Don't pass to agent
          }
        } catch (err) {
          // If interception fails, continue with normal processing
          logVerboseMessage(`matrix: krill interception error: ${err}`);
        }
      }
