...
async function fetchMatrixMediaBuffer(params: {
  ...
  // PATCH: matrix-bot-sdk 0.8.0 returns {data, contentType} not Buffer directly
  try {
    const result = await params.client.downloadContent(params.mxcUrl);
    // Handle both old (Buffer) and new ({data, contentType}) return types
    const buffer = (result as any).data ?? result;
    if (buffer.byteLength > params.maxBytes) {
      throw new Error("Matrix media exceeds configured size limit");
    }
    return { 
      buffer: Buffer.from(buffer), 
      headerType: (result as any).contentType 
    };
  } catch (err) {
