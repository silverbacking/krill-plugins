...
import type {
  SenseContext, TranscriptChunk, WakeWordEvent, AudioSession
} from "./types.js";

const DEFAULT_CONTEXT_WINDOW = 60; // seconds
const MAX_DAILY_LINES = 5000;

// In-memory buffer for recent transcript (for building context on wake word)
interface TranscriptEntry {
  text: string;
  timestamp: number; // epoch ms
}
const recentTranscript: TranscriptEntry[] = [];
const MAX_BUFFER_ENTRIES = 200;

function getAudioDir(ctx: SenseContext): string {
...
function getSessionPath(ctx: SenseContext): string {
  return path.join(getAudioDir(ctx), "current-session.json");
}

function getConfigPath(ctx: SenseContext): string {
  return path.join(getAudioDir(ctx), "config.json");
}

function getDailyTranscriptPath(ctx: SenseContext): string {
 ...
 */
function bufferTranscript(text: string, timestamp: string): void {
 ...
 */
function getRecentContext(windowSeconds: number): string {
 ...
 */
function appendToDaily(ctx: SenseContext, text: string, timestamp: string): void {
 ...
 */
async function handleTranscriptChunk(ctx: SenseContext, chunk: TranscriptChunk): Promise<void> {
  ...
  // Update session
  const session = readJSON<AudioSession>(getSessionPath(ctx), { active: true });
 ...
 */
async function handleWakeWord(ctx: SenseContext, event: WakeWordEvent): Promise<void> {
 ...
 */
async function handleConfig(ctx: SenseContext, config: any): Promise<void> {
 ...
 */
async function handleSession(ctx: SenseContext, kind: string, content: any): Promise<void> {
 ...
 */
export async function handleAudio(ctx: SenseContext): Promise<void> {
  ...
  switch (kind) {
    case "transcript_chunk":
      await handleTranscriptChunk(ctx, content as TranscriptChunk);
      break;
      
    case "wake_word":
      await handleWakeWord(ctx, content as WakeWordEvent);
      break;
