 ...
 */
function cleanupExpiredSessions(ctx: SenseContext): void {
 ...
 */
export async function handleMicrophone(ctx: SenseContext): Promise<void> {
  const event = ctx.content as MicrophoneEvent;
  ...
  ctx.logger.info(
    `[senses/microphone] Part ${event.part}, final=${event.final}, session=${event.sessionId.substring(0, 8)}…, file=${event.fileName}`
  );
