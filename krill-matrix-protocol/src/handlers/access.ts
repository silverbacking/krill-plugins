...
export interface AccessState {
  // Users who have completed PIN verification
  verified: {
    [mxid: string]: {
      verifiedAt: number;
      userId?: string;  // Krill user ID if known
      email?: string;
    };
  };
  // Users currently being prompted for PIN
  pendingPin: {
    [mxid: string]: {
      promptedAt: number;
      attempts: number;
...
let options: AccessHandlerOptions | null = null;
let state: AccessState = { verified: {}, pendingPin: {} };
 ...
 */
export function initAccessHandler(opts: AccessHandlerOptions): void {
  options = opts;
  loadState();
 ...
 */
export function isVerified(mxid: string): boolean {
 ...
 */
export function isPendingPin(mxid: string): boolean {
 ...
 */
async function verifyPinWithApi(mxid: string, pin: string): Promise<{ valid: boolean; userId?: string; email?: string }> {
 ...
 */
export function markVerified(mxid: string, userId?: string, email?: string): void {
  delete state.pendingPin[mxid];
  state.verified[mxid] = {
    verifiedAt: Date.now(),
    userId,
 ...
 */
export function revokeAccess(mxid: string): void {
