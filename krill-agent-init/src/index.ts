    ...
    try {
      ...
      if (entries && typeof entries === "object") {
        for (const key of Object.keys(entries)) {
          if (key !== "krill-agent-init") plugins.push(key);
        }
      }
    } catch {}
    ...
    try {
      ...
      if (model === "unknown" || typeof model === "object") {
        ...
        if (fs.existsSync(configPath)) {
          ...
          else if (typeof m === "object" && m?.primary) model = m.primary;
        }
      }
    } catch {}
...
async function doCheckin(
  ...
  systemInfo: { os: string; arch: string; hostname: string; node_version: string; openclaw_version: string },
  plugins: string[],
  model: string
) {
  const checkinBody = {
  ...
  try {
    ...
    if (res.ok) {
      api.logger.info(
        `[krill-init] ✅ Check-in OK: ${config.gatewayId} | ${systemInfo.os} ${systemInfo.arch} | OC ${systemInfo.openclaw_version}`
      );
      return;
    }
