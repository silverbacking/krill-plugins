...
interface UpdateConfig {
  ...
  // Skills settings
  skillsPath: string;  // Where to install skills (default: ~/skills or workspace/skills)
}
 ...
 */
function scanInstalledSkills(): void {
  const skillsPath = getSkillsPath();
  if (!skillsPath || !existsSync(skillsPath)) return;
...
function getSkillsPath(): string {
  if (pluginConfig.skillsPath) return pluginConfig.skillsPath;
 ...
 */
async function installSkill(update: PluginUpdate): Promise<boolean> {
  const logger = pluginApi?.logger;
  const skillsPath = getSkillsPath();
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ version: update.version, installed_at: new Date().toISOString() }, null, 2)
    );
...
async function checkForUpdates(): Promise<void> {
  ...
  try {
    ...
    scanInstalledSkills();
    const installedSkillsObj: Record<string, string> = {};
    ...
    if (skill_updates?.length > 0) {
      ...
      for (const update of skill_updates) {
        logger?.info(
          `[krill-update] Skill available: ${update.plugin} ${update.current} → ${update.latest}`
        );
